...
export type PopbillMemberCreateBody = z.infer<typeof createBodySchema>;
export const POST = handleApi(async (req, context) => {
  ...
  if (company.popbillId)
    throw new ConflictError("이미 팝빌 연동되어 있는 고객사");
  ...
  const result = await new Promise((res, rej) => {
    axios
      .post(`${process.env.POPBILL_API_URL}/TaxinvoiceService/JoinMember`, body)
      .then((result) => {
        res(result.data);
      })
      .catch((err) => {
