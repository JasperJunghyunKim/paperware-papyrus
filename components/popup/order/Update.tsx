...
export default function Component(props: Props) {
  ...
  const apiContinue = Api.External.Order.useOrderRequest(
    order.data?.id ?? null
  );
        ...
        {order.data && Util.inc(order.data.status, "PREPARING") && (
          <Toolbar.Container>
            ...
            {!isVirtual && (
              <>
                <Toolbar.ButtonPreset.Send
                  ...
                  onClick={cmdContinue}
                  disabled={
                    !Util.inc(order.data.status, "PREPARING", "REJECTED")
                  }
                />
              </>
            )}
            {isVirtual && (
              <Toolbar.ButtonPreset.Send
                label="발주 진행"
                tooltip="가상 매입처를 대상으로 작성된 주문은 발주 과정을 건너뜁니다."
                onClick={cmdContinue}
              />
            )}
          </Toolbar.Container>
