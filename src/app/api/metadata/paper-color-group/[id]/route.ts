...
import { z } from "zod";
import { PaperColorGroup } from "../route";
...
export const GET = handleApi(async (req, context) => {
  ...
  const data = await prisma.paperColorGroup.findUnique({
    where: { id: params.id },
  });
...
export const PUT = handleApi(async (req, context) => {
  ...
  return await prisma.paperColorGroup.update({
    where: {
      id: params.id,
    },
    data: {
...
export type GetPaperColorGroupItemResponse = PaperColorGroup;
export type UpdatePaperColorGroupBody = {
  name: string;
};
