...
export const GET = handleApi(async (req, context) => {
  ...
  const data = await prisma.paperGroup.findUnique({
    where: { id: params.id },
  });
...
export const PUT = handleApi(async (req, context) => {
  ...
  return await prisma.paperGroup.update({
    where: {
      id: params.id,
    },
    data: {
...
export type GetPaperGroupItemResponse = PaperGroup;
export type UpdatePaperGroupBody = {
  name: string;
};
