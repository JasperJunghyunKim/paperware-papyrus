...
export const GET = handleApi<GetPopbillCompanyInfoReseponse>(
  async (req, context) => {
    ...
    const result = await new Promise((res, rej) => {
      axios
        .get(
          `${process.env.POPBILL_API_URL}/TaxinvoiceService/GetCorpInfo?companyRegistrationNumber=${company.companyRegistrationNumber}`
        )
        .then((result) => {
          res(result.data);
        })
...
export const DELETE = handleApi(async (req, context) => {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const result = await new Promise((res, rej) => {
      axios
        .post(`${process.env.POPBILL_API_URL}/TaxinvoiceService/QuitMember`, {
